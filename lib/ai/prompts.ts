...
export const SCENES = [
  ...
  },
  {
    ...
    videoPrompt: VIDEO_PROMPTS[3],
  },
] as const;

export function getScenesForCity(city?: string | null) {
  const loc = getCityLocation(city);
  return [
    {
      label: "Sahne 1",
      imagePrompt: `A handsome man wearing an elegant tailored dark navy suit, running across ${loc.s1}, dust kicking up behind his polished shoes. A beautiful woman in a flowing light dress runs behind him, chasing him with a determined yet playful expression. Dramatic late afternoon sun creating long shadows. Both faces clearly visible, identity preserved from reference images. ${COMMON_STYLE}`,
      videoPrompt: VIDEO_PROMPTS[0],
    },
    {
      label: "Sahne 2",
      imagePrompt: `The woman reaches the man and gently grabs his hand from behind. The exact moment of contact: the world around them transforms — ${loc.s2}, blooming wildflowers, towering trees with emerald leaves, a magical Eden-like garden materializes around them. Soft sunbeams filter through new canopy. Both characters frozen in this magical moment of touch. ${COMMON_STYLE}`,
      videoPrompt: VIDEO_PROMPTS[1],
    },
    {
      label: "Sahne 3",
      imagePrompt: `The man, still in his elegant suit, turns to face the woman and looks deeply into her eyes with an emotional, love-struck expression. Soft tears of joy in his eyes. They stand in the heart of the lush green paradise, sunlight kissing their faces, ${loc.s3}. Intimate medium close-up shot, both faces clearly visible and emotionally expressive. ${COMMON_STYLE}`,
      videoPrompt: VIDEO_PROMPTS[2],
    },
    {
      label: "Sahne 4",
      imagePrompt: `The man and the woman hold hands and walk together deeper into the paradise garden, shot from behind in cinematic wide angle, ${loc.s4}. Suddenly, in the foreground, a magnificent thornless red rose grows rapidly from the ground, its petals unfurling in extreme detail, dominating the frame. The couple is visible through the petals, walking into the light. ${COMMON_STYLE}`,
      videoPrompt: VIDEO_PROMPTS[3],
    },
  ];
}
